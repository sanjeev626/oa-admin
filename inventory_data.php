...
<form method="POST">
 <table cellpadding="0" cellspacing="0" border="0" width="100%" class="FormTbl" style="font-size:12px;">     
	...
	while($rasPurchase = $mydb->fetch_array($result))
	{
		...
		$result2 = $mydb->getQuery('tst.*,tmd.category','tbl_stock tst INNER JOIN tbl_medicine tmd ON tmd.id=tst.medicine_id','tst.creditmemo_id='.$creditmemo_id.' AND tst.sales<tst.stock GROUP BY tst.medicine_id' );
		...
		<tr>
		  ...
		  <td align="center" class="titleBarA" valign="top"><?php echo $rasOrder['stock']-$rasOrder['sales'];?></td>
