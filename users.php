...
<script type="text/javascript">
 function callDelete(gid)
 {
	...
	{
	window.location='?manager=users&did='+gid;
	}
 }
</script>
...
{
	$delId = $_GET['did'];
    $count_sales = $mydb->getCount('id','tbl_sales','client_id='.$delId);
    if($count_sales==0)
    {
        $mydb->deleteQuery('tbl_image','client_id='.$delId);
        $mess = $mydb->deleteQuery('users','id='.$delId);        
    }
    else
        $mess = "This client has already made purchase with us so cannot be deleted.";
    $url = ADMINURLPATH."users&message=".$mess;
    $mydb->redirect($url);
}
...
?>
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="FormTbl" style="font-size:12px;">
        <tr class="TitleBar">
            <td colspan="2" class="TtlBarHeading">Users
                <div style="float:right"></div>
            </td>
            <td colspan="5" class="TtlBarHeading" width="100%"><input name="search" type="text" id="usersearch" Placeholder="Search by Name" class="button" style="width:250px;"/></td>
        ...
        <tr>
            ...
            <td width="15%" class="titleBarB" align="center"><strong>Name</strong></td>
            <td width="20%" class="titleBarB" align="center"><strong>Email</strong></td>
            <td width="20%" class="titleBarB" align="center"><strong>Address</strong></td>
            <td width="15%" class="titleBarB" align="center"><strong>Contact Number</strong></td>
            <td width="13%" class="titleBarB" align="center"><strong>Reference</strong></td>
        ...
        while ($rasMember = $mydb->fetch_array($result1))
        {
            ...
            <tr id="userinfo<?php echo $rasMember['id']; ?>">
                ...
                <td class="titleBarA" valign="top"><?php echo $rasMember['phone']; if(!empty($rasMember['additional_phone'])) echo " / ".$rasMember['additional_phone'];?></td>
...
else
{
    ...
    $result = $mydb->getQuery('*', 'users', 'active=1  ORDER BY name limit ' . $start_from . ',' . $num_rec_per_page . '');
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="FormTbl" style="font-size:12px;">
        <tr class="TitleBar">
            <td colspan="2" class="TtlBarHeading">Users (<?php echo $mydb->getCount('id', 'users','active=1');?>)
                <div style="float:right"></div>
            </td>
    ...
    if (isset($page)) {
        ...
        <nav style="text-align:center;">
            <ul class="pagination">
                <?php
                $rs_result = $mydb->getCount('id', 'users','active=1');
                ...
                else
                {
                    $j = $page - 1;
                    echo '<li >
                            <a href="' . ADMINURLPATH . 'users&page=' . $j . '" aria-label="Previous">
                ...
                for ($i = 1; $i <= $total_pages; $i++) {
                    if ($i <> $page) {
                        ...
                        echo '<li><a href ="' . ADMINURLPATH . 'users&page=' . $i . '" >' . $i . '</a></li>';
                    } else {
                        echo '<li >
                ...
                if ($page == $total_pages) {
                    ...
                    $j = $page + 1;
                    echo "<li>
                    <a href='" . ADMINURLPATH . 'users&page=' . $j . "' aria-label='Next'>
