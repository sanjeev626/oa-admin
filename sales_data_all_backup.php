...
$query = $mydb->getQuery('ts.*,tc.name','tbl_sales ts INNER JOIN users tc ON tc.id=ts.client_id','discount_amount="0" AND delivery_date BETWEEN "'.$start_date.'" AND "'.$end_date.'" ORDER BY delivery_date ASC','1');
echo $query;
$result = $mydb->getQuery('ts.*,tc.name','tbl_sales ts INNER JOIN users tc ON tc.id=ts.client_id','(discount_amount="0" OR discount_amount="")  AND delivery_date BETWEEN "'.$start_date.'" AND "'.$end_date.'" ORDER BY delivery_date ASC');
//exit();
?>
<form method="POST">
 <table cellpadding="0" cellspacing="0" border="0" width="100%" class="FormTbl" style="font-size:12px;">     
	...
	while($rasSales = $mydb->fetch_array($result))
	{
		...
		<tr>
		  ...
		  <td class="titleBarA" valign="top"><?php echo $rasSales['name'];?></td>
