...
if(isset($_GET['do']) && $_GET['do']=="paid_n_delivered")
{    
    ...
    $mess = $mydb->updateQuery('tbl_sales',$data,'id='.$sales_id);

    $data1='';
    $data1['status']=1;
    //$data1['refill_day'] = $refill;
    $update_order=$mydb->updateQuery('tbl_order',$data1,'sales_id='.$sales_id);  

    $url = ADMINURLPATH.'counter_sales_details&sales_id='.$sales_id;    
    $mydb->redirect($url);
}
...
?>
  ...
  ?>
   <tr>
    ...
    <tr>
      <td class="titleBarA" colspan="6" valign="top" style="text-align:right;"><?php if($rasSales['payment']!="paid"){?><input type="button" name="btnEdit" id="btnEdit" value="Paid & Delivered" class="button" onclick="window.location='<?php echo SITEROOTADMIN.ADMINURLPATH?>counter_sales_details&sales_id=<?php echo $sales_id;?>&do=paid_n_delivered';"><?php } ?></td>
      <td class="titleBarA" valign="top">
