...
if(isset($_GET['reorder_id']))
{
  ...
  $result1 =$mydb->getQuery('*','tbl_order','sales_id="'.$id.'" And status=1');      
...
{   
  ...
  while($quantity_by_user>0)
 {
        ...
        $data1['medicine_type']=$med_dose;
        $rasStock = $mydb->getArray('id,pack,rate,stock,sales','tbl_stock','description="'.$compmed_sales1.'" AND sales<>stock group by description');
        ...
        {   
              ...
              else
              {
                  ...
                  $quantity_by_user=$quantity_by_user-$available;//130-120
